...
void displayList(ClassListPtr);          //Display student information
void findElem(ClassListPtr);             //Find student by ID and Display
void deleteStudet(ClassListPtr);         //delete student in List
void sortList(ClassListPtr);             //Sort the list(selection sort)
void addStudent(ClassListPtr);           //addstudent at last index
void addStudent_First(ClassListPtr);     //addstudent at first index
void addStudent_inSort(ClassListPtr);    //addstudent at correct index in sorted list
ClassList same_birthyear(ClassList);     //inialization function for student with same year
void displayList_sameyear(ClassListPtr);

int main(){
    ClassListPtr IT = (ClassListPtr)malloc(sizeof(ClassList));
    IT->ctr = 0;
    IT->max = 10;
...
void addStudent(ClassListPtr first){
...
void displayList(ClassListPtr first){
    int i;
    printf("===STUDENTS===\n");
    if(first->ctr !=0){
...
void findElem(ClassListPtr first){
    string id;
    printf("Enter Student ID: ");
    scanf("%s",id);
    int i,flag = 1;
...
void deleteStudet(ClassListPtr first){
    string lname;
...
void sortList(ClassListPtr first){
    int i,j;
...
void addStudent_First(ClassListPtr first){
    if(first->ctr < first->max){
        int i;
...
void addStudent_inSort(ClassListPtr first){
    addStudent(first);
    int i,j;
    for(i=0;i<first->ctr-1;i++){
        int min = i;
...
void displayList_sameyear(ClassListPtr first){
    int i;
    printf("===STUDENTS===\n");
    if(first->ctr !=0){
